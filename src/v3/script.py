...
@click.command()
@click.option('--controller-ip', '-ip', type=str, help='IP del controlador remoto', required=True)
@click.option('--controller-port', '-port', type=int, help='Puerto del controlador remoto', required=True)
def main(controller_ip, controller_port):
    setLogLevel('info')

    topo = MyTopo(controller_ip, controller_port)
